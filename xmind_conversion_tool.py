...
class XMindToExcelApp:
    def __init__(self, root):
        ...
        self.scenario_sub.grid(row=2, column=1, padx=10, pady=10, sticky="w")

        # Affected Version
        tk.Label(root, text="影响版本").grid(row=3, column=0, padx=10, pady=10, sticky="w")
        self.version = ttk.Combobox(root, values=["4.1", "4.2"], state="readonly")
        self.version.set("4.2")
        self.version.grid(row=3, column=1, padx=10, pady=10, sticky="w")

        # Test Case Level
        tk.Label(root, text="用例级别").grid(row=4, column=0, padx=10, pady=10, sticky="w")
        self.level = ttk.Combobox(root, values=["未分级", "1", "2", "3", "4"], state="readonly")
        self.level.set("未分级")
        self.level.grid(row=4, column=1, padx=10, pady=10, sticky="w")

        # Test Case Type
        tk.Label(root, text="用例类型").grid(row=5, column=0, padx=10, pady=10, sticky="w")
        ...
        # Tags
        tk.Label(root, text="标签").grid(row=6, column=0, padx=10, pady=10, sticky="w")
        self.tags = tk.Entry(root, width=50)
        self.tags.grid(row=6, column=1, padx=10, pady=10)

        # Link Type
        tk.Label(root, text="链接类型").grid(row=7, column=0, padx=10, pady=10, sticky="w")
        self.link_type = tk.StringVar(value="关联的任务")
        tk.Radiobutton(root, text="关联的任务", variable=self.link_type, value="关联的任务").grid(row=7, column=1,
                                                                                                  sticky="w")
        tk.Radiobutton(root, text="复制的原问题", variable=self.link_type, value="复制的原问题").grid(row=7, column=1,
                                                                                                      padx=150,
                                                                                                      sticky="w")
        tk.Label(root, text="问题").grid(row=7, column=2, padx=10, sticky="w")
        self.issue = tk.Entry(root, width=20)
        self.issue.grid(row=7, column=3, padx=10, pady=10)

        # Preview Button
        tk.Button(root, text="预览", command=self.preview_test_cases).grid(row=8, column=0, columnspan=4, pady=20)
    ...
    def preview_test_cases(self):
        if not self.xmind_path.get() or not self.scenario_main.get() or not self.version.get():
            messagebox.showerror("错误", "请确保xmind文件位置、功能场景和影响版本已填写！")
            return

        try:
            manager = TestCaseManager(
                xmind_file=self.xmind_path.get(),
                output_dir="",
                scenario_main=self.scenario_main.get(),
                scenario_sub=self.scenario_sub.get(),
    ...
    def show_preview_table(self):
        ...
        self.table_frame.grid(row=9, column=0, columnspan=4, pady=10)

        # 基本字段和附加信息
        additional_data = {
            "功能场景": self.scenario_main.get(),
            "影响版本": self.version.get(),
            "用例级别": self.level.get(),
        ...
        # 使用 pandastable 展示
        self.table = Table(self.table_frame, dataframe=df, editable=True)
        self.table.show()

        # 添加操作按钮
        tk.Button(self.root, text="生成Excel", command=self.generate_excel).grid(row=10, column=0, columnspan=2,
                                                                                 pady=10)
